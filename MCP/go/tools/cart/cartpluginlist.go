...
func CartpluginlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/cart.plugin.list.json%s", cfg.BaseURL, queryString)
...
func CreateCartpluginlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_cart_plugin_list_json",
		mcp.WithDescription("Get list of installed plugins"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CartpluginlistHandler(cfg),
	}
}
