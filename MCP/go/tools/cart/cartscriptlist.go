...
func CartscriptlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["script_ids"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("script_ids=%v", val))
		}
		...
		url := fmt.Sprintf("%s/cart.script.list.json%s", cfg.BaseURL, queryString)
		...
		var result models.ModelResponseCartScriptList
...
func CreateCartscriptlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_cart_script_list_json",
		mcp.WithDescription("Get scripts installed to the storefront"),
		...
		mcp.WithString("script_ids", mcp.Description("Retrieves only scripts with specific ids")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CartscriptlistHandler(cfg),
	}
}
