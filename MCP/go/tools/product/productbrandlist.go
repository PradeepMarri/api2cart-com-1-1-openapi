...
func ProductbrandlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["brand_ids"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("brand_ids=%v", val))
		}
		...
		url := fmt.Sprintf("%s/product.brand.list.json%s", cfg.BaseURL, queryString)
...
func CreateProductbrandlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_product_brand_list_json",
		mcp.WithDescription("Get list of brands from your store."),
		...
		mcp.WithString("brand_ids", mcp.Description("Retrieves brands specified by brand ids")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductbrandlistHandler(cfg),
	}
}
