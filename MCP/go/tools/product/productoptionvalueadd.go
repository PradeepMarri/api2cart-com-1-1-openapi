...
func ProductoptionvalueaddHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/product.option.value.add.json%s", cfg.BaseURL, queryString)
...
func CreateProductoptionvalueaddTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_product_option_value_add_json",
		mcp.WithDescription("Add product option item from option."),
		mcp.WithString("product_id", mcp.Required(), mcp.Description("Defines product id where the option value should be added")),
		mcp.WithString("option_id", mcp.Required(), mcp.Description("Defines option id where the value has to be added")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductoptionvalueaddHandler(cfg),
	}
}
