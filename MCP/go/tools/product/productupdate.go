...
func ProductupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ProductUpdate
		...
		url := fmt.Sprintf("%s/product.update.json", cfg.BaseURL)
...
func CreateProductupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_product_update_json",
		mcp.WithDescription("Update price and quantity for a specific product"),
		mcp.WithString("reserve_quantity", mcp.Description("Input parameter: This parameter allows to reserve/unreserve product quantity.")),
		...
		mcp.WithString("name", mcp.Description("Input parameter: Defines product's name that has to be updated")),
		mcp.WithBoolean("in_stock", mcp.Description("Input parameter: Set stock status")),
		mcp.WithString("special_price", mcp.Description("Input parameter: Defines new product's special price")),
		mcp.WithString("manufacturer", mcp.Description("Input parameter: Defines product's manufacturer")),
		mcp.WithString("report_request_id", mcp.Description("Input parameter: Report request id")),
		mcp.WithString("cost_price", mcp.Description("Input parameter: Defines new product's cost price")),
		mcp.WithString("model", mcp.Description("Input parameter: Defines product model that has to be updated")),
		...
		mcp.WithString("price", mcp.Description("Input parameter: Defines new product's price")),
		...
		mcp.WithString("id", mcp.Description("Input parameter: Defines product id that has to be updated")),
		...
		mcp.WithString("quantity", mcp.Description("Input parameter: Defines new product's quantity")),
		mcp.WithString("country_of_origin", mcp.Description("Input parameter: The country where the inventory item was made")),
		mcp.WithBoolean("disable_report_cache", mcp.Description("Input parameter: Disable report cache for current request")),
		mcp.WithString("sprice_create", mcp.Description("Input parameter: Defines the date of special price creation")),
		mcp.WithString("store_id", mcp.Description("Input parameter: Defines store id where the product should be found")),
		...
		mcp.WithString("sku", mcp.Description("Input parameter: Defines new product's sku")),
		mcp.WithString("manufacturer_id", mcp.Description("Input parameter: Defines product's manufacturer by manufacturer_id")),
		...
		mcp.WithString("description", mcp.Description("Input parameter: Defines new product's description")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductupdateHandler(cfg),
	}
}
