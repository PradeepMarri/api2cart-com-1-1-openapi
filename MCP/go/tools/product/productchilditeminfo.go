...
func ProductchilditeminfoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/product.child_item.info.json%s", cfg.BaseURL, queryString)
...
func CreateProductchilditeminfoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_product_child_item_info_json",
		mcp.WithDescription("Get child for specific product."),
		...
		mcp.WithString("product_id", mcp.Required(), mcp.Description("Filter by parent product id")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductchilditeminfoHandler(cfg),
	}
}
