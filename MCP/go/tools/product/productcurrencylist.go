...
func ProductcurrencylistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/product.currency.list.json%s", cfg.BaseURL, queryString)
...
func CreateProductcurrencylistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_product_currency_list_json",
		mcp.WithDescription("Get list of currencies"),
		...
		mcp.WithBoolean("default", mcp.Description("Specifies the set of default/not default currencies")),
		mcp.WithBoolean("avail", mcp.Description("Specifies the set of available/not available currencies")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ProductcurrencylistHandler(cfg),
	}
}
