...
func ProductreviewlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/product.review.list.json%s", cfg.BaseURL, queryString)
...
func CreateProductreviewlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_product_review_list_json",
		mcp.WithDescription("Get reviews of a specific product."),
		...
		mcp.WithString("ids", mcp.Description("Retrieves reviews specified by ids")),
		mcp.WithString("store_id", mcp.Description("Store Id")),
		mcp.WithString("status", mcp.Description("Defines status")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductreviewlistHandler(cfg),
	}
}
