...
func ProductchilditemfindHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/product.child_item.find.json%s", cfg.BaseURL, queryString)
...
func CreateProductchilditemfindTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_product_child_item_find_json",
		mcp.WithDescription("Search product child item (bundled item or configurable product variant) in store catalog."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductchilditemfindHandler(cfg),
	}
}
