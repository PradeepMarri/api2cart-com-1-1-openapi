...
func ProductvariantcountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/product.variant.count.json%s", cfg.BaseURL, queryString)
...
func CreateProductvariantcountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_product_variant_count_json",
		mcp.WithDescription("Get count variants."),
		...
		mcp.WithString("category_id", mcp.Description("Counts products’ variants specified by category id")),
		mcp.WithString("product_id", mcp.Required(), mcp.Description("Retrieves products' variants specified by product id")),
		mcp.WithString("store_id", mcp.Description("Retrieves variants specified by store id")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ProductvariantcountHandler(cfg),
	}
}
