...
func ProductoptionlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/product.option.list.json%s", cfg.BaseURL, queryString)
...
func CreateProductoptionlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_product_option_list_json",
		mcp.WithDescription("Get list of options."),
		...
		mcp.WithString("product_id", mcp.Description("Retrieves products' options specified by product id")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductoptionlistHandler(cfg),
	}
}
