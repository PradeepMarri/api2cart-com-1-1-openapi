...
func ProductchilditemlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/product.child_item.list.json%s", cfg.BaseURL, queryString)
		...
		var result models.ModelResponseProductChildItemList
...
func CreateProductchilditemlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_product_child_item_list_json",
		mcp.WithDescription("Get child items list of specific product(s)."),
		mcp.WithString("page_cursor", mcp.Description("Used to retrieve products child items via cursor-based pagination (it can't be used with any other filtering parameter)")),
		...
		mcp.WithString("product_id", mcp.Description("Filter by parent product id")),
		mcp.WithString("product_ids", mcp.Description("Filter by parent product ids")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductchilditemlistHandler(cfg),
	}
}
