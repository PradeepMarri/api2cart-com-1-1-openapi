...
func OrderabandonedlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["skip_empty_email"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("skip_empty_email=%v", val))
		}
		...
		url := fmt.Sprintf("%s/order.abandoned.list.json%s", cfg.BaseURL, queryString)
		...
		var result models.ModelResponseOrderAbandonedList
...
func CreateOrderabandonedlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_order_abandoned_list_json",
		mcp.WithDescription("Get list of orders that were left by customers before completing the order."),
		...
		mcp.WithBoolean("skip_empty_email", mcp.Description("Filter empty emails")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    OrderabandonedlistHandler(cfg),
	}
}
