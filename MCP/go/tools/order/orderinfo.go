...
func OrderinfoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/order.info.json%s", cfg.BaseURL, queryString)
...
func CreateOrderinfoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_order_info_json",
		mcp.WithDescription("Info about a specific order by ID"),
		mcp.WithString("order_id", mcp.Description("Retrieves order’s info specified by order id")),
		mcp.WithString("id", mcp.Description("Retrieves order info specified by id")),
		...
		mcp.WithBoolean("enable_cache", mcp.Description("If the value is 'true' and order exist in our cache, we will return order.info response from cache")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    OrderinfoHandler(cfg),
	}
}
