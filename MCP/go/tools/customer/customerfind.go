...
func CustomerfindHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/customer.find.json%s", cfg.BaseURL, queryString)
...
func CreateCustomerfindTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_customer_find_json",
		mcp.WithDescription("Find customers in store."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CustomerfindHandler(cfg),
	}
}
