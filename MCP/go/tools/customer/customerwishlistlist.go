...
func CustomerwishlistlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/customer.wishlist.list.json%s", cfg.BaseURL, queryString)
...
func CreateCustomerwishlistlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_customer_wishlist_list_json",
		mcp.WithDescription("Get a Wish List of customer from the store."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CustomerwishlistlistHandler(cfg),
	}
}
