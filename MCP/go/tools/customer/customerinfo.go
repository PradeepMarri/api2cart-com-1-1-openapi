...
func CustomerinfoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/customer.info.json%s", cfg.BaseURL, queryString)
...
func CreateCustomerinfoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_customer_info_json",
		mcp.WithDescription("Get customers' details from store."),
		mcp.WithString("id", mcp.Required(), mcp.Description("Retrieves customer's info specified by customer id")),
		...
		mcp.WithString("store_id", mcp.Description("Retrieves customer info specified by store id")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CustomerinfoHandler(cfg),
	}
}
