...
func SubscriberlistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["subscribed"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("subscribed=%v", val))
		}
		...
		url := fmt.Sprintf("%s/subscriber.list.json%s", cfg.BaseURL, queryString)
...
func CreateSubscriberlistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriber_list_json",
		mcp.WithDescription("Get subscribers list"),
		...
		mcp.WithBoolean("subscribed", mcp.Description("Filter by subscription status")),
		mcp.WithString("store_id", mcp.Description("Store Id")),
		mcp.WithString("email", mcp.Description("Filter subscribers by email")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    SubscriberlistHandler(cfg),
	}
}
