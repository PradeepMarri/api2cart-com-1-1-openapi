...
func CategoryfindHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/category.find.json%s", cfg.BaseURL, queryString)
...
func CreateCategoryfindTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_category_find_json",
		mcp.WithDescription("Search category in store. "Laptop" is specified here by default."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CategoryfindHandler(cfg),
	}
}
