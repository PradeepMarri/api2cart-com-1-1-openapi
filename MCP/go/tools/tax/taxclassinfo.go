...
func TaxclassinfoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["tax_class_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("tax_class_id=%v", val))
		}
		...
		url := fmt.Sprintf("%s/tax.class.info.json%s", cfg.BaseURL, queryString)
...
func CreateTaxclassinfoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_tax_class_info_json",
		mcp.WithDescription("Get info about tax"),
		mcp.WithString("tax_class_id", mcp.Required(), mcp.Description("Retrieves taxes specified by class id")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    TaxclassinfoHandler(cfg),
	}
}
