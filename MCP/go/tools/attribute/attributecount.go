...
func AttributecountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/attribute.count.json%s", cfg.BaseURL, queryString)
...
func CreateAttributecountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_attribute_count_json",
		mcp.WithDescription("Get attributes count"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    AttributecountHandler(cfg),
	}
}
