...
func AttributeupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/attribute.update.json%s", cfg.BaseURL, queryString)
...
func CreateAttributeupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_attribute_update_json",
		mcp.WithDescription("Update attribute data"),
		mcp.WithString("id", mcp.Required(), mcp.Description("Entity id")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Defines new attributes's name")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    AttributeupdateHandler(cfg),
	}
}
