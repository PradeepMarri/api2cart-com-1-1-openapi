...
func AttributeinfoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/attribute.info.json%s", cfg.BaseURL, queryString)
...
func CreateAttributeinfoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_attribute_info_json",
		mcp.WithDescription("Get attribute info"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    AttributeinfoHandler(cfg),
	}
}
