...
func AttributelistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["attribute_ids"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("attribute_ids=%v", val))
		}
		...
		if val, ok := args["system"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("system=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/attribute.list.json%s", cfg.BaseURL, queryString)
		...
		var result models.ModelResponseAttributeList
...
func CreateAttributelistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_attribute_list_json",
		mcp.WithDescription("Get attributes list"),
		...
		mcp.WithString("type", mcp.Description("Defines attribute's type")),
		mcp.WithString("attribute_ids", mcp.Description("Filter attributes by ids")),
		mcp.WithString("store_id", mcp.Description("Store Id")),
		mcp.WithString("lang_id", mcp.Description("Retrieves attributes on specified language id")),
		...
		mcp.WithBoolean("visible", mcp.Description("Filter items by visibility status")),
		mcp.WithBoolean("required", mcp.Description("Defines if the option is required")),
		mcp.WithBoolean("system", mcp.Description("True if attribute is system")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AttributelistHandler(cfg),
	}
}
